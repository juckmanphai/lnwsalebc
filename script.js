...
// --- [ใหม่] ฟังก์ชันจัดการการสแกนบาร์โค้ด ---
    handleBarcodeScan(barcode) {
      if (!barcode) return;
      
      // ค้นหาสินค้าที่มีบาร์โค้ดตรงกัน
      ...
      if (product) {
          ...
          // เพิ่มลงตะกร้า 1 ชิ้น
          this._addProductToCartLogic(product, 1);
          
          this.showToast(`เพิ่ม "${product.name}" แล้ว`);
      } else {
          this.showToast(`ไม่พบสินค้าที่มีบาร์โค้ด: ${barcode}`, "warning");
      }
    },

    // --- [ใหม่] Logic การเพิ่มสินค้าลงตะกร้า (ใช้ร่วมกันทั้งแบบ Manual และ Barcode) ---
    _addProductToCartLogic(product, quantity) {
      // เช็คว่ามีสินค้านี้ในตะกร้าแล้วหรือยัง (เช็คราคาปกติ ไม่ใช่ราคาพิเศษ)
      const existingCartItem = this.cart.find(
          (item) => item.id === product.id && !item.isSpecialPrice
      );
    ...
    // --- [แก้ไข] renderPos ให้ Focus ช่อง Barcode และรองรับ HTML ใหม่ ---
    renderPos(payload = null) {
      ...
      if (this.currentUser.role === "seller" && productsInStock.length === 1) {
        ...
        productSelect.innerHTML =
          '<option value="">--- เลือกสินค้า (หรือสแกนบาร์โค้ด) ---</option>';
      ...
      if (payload) {
        ...
        if (payload.paymentMethod === "เครดิต") {
          const creditRadio = document.querySelector('input[name="payment-method"][value="เครดิต"]');
          if(creditRadio) creditRadio.checked = true;
          
          const buyerInput = document.getElementById("credit-buyer-name");
          if(buyerInput) buyerInput.value = payload.buyerName || "";
          ...
          if (payload.creditDueDate && payload.date) {
            ...
            const daysInput = document.getElementById("credit-due-days");
            if(daysInput) daysInput.value = dayDiff >= 0 ? dayDiff : "";
          }
        } else if (payload.paymentMethod === "เงินโอน") {
          const transferRadio = document.querySelector('input[name="payment-method"][value="เงินโอน"]');
          if(transferRadio) transferRadio.checked = true;
          
          const transferInput = document.getElementById("transfer-name");
          if(transferInput) transferInput.value = payload.transferorName || "";
        } else {
          const cashRadio = document.querySelector('input[name="payment-method"][value="เงินสด"]');
...
// --- [แก้ไข] addToCart ให้เรียกใช้ Logic กลาง ---
    addToCart(e) {
      e.preventDefault();
      ...
      // ตรวจสอบว่ามีการกรอกราคาพิเศษหรือไม่
      if (
        specialPriceInput &&
        specialPriceInput.parentElement.parentElement.style.display !== "none" &&
        specialPriceInput.value.trim() !== ""
      ) {
        // กรณีราคาพิเศษ: ใช้ Logic เดิม (แยกรายการออกมา)
        if (quantity > product.stock) {
           this.showToast("สินค้าในสต็อกไม่เพียงพอ");
           return;
        }
        ...
        // เช็คว่ามีรายการราคาพิเศษนี้อยู่แล้วหรือยัง
        const existingCartItem = this.cart.find(
          (item) => item.id === product.id && item.sellingPrice === sellingPrice && item.isSpecialPrice === true
        );
        ...
        this.renderCart();
         ...
         // กรณีราคาปกติ: เรียกใช้ฟังก์ชันกลาง
         this._addProductToCartLogic(product, quantity);
      }
      ...
      // Focus กลับไปที่ช่อง Barcode
// Focus กลับไปที่ช่อง Barcode
      const barcodeInput = document.getElementById("pos-barcode-input");
      if(barcodeInput) barcodeInput.focus();
    },

    // --- ฟังก์ชันที่ขาดหายไป 1: ลบสินค้าออกจากตะกร้า ---
    removeFromCart(indexStr) {
    ...
    // --- ฟังก์ชันที่ขาดหายไป 2: ยืนยันการขาย (Process Sale) ---
    async processSale() {
      if (this.cart.length === 0) {
        this.showToast("ตะกร้าสินค้าว่างเปล่า", "warning");
        return;
      }
      ...
      try {
        ...
        for (const item of this.cart) {
          ...
          if (!product) throw new Error(`ไม่พบสินค้า ${item.name} ในระบบ`);
          if (product.stock < item.quantity) throw new Error(`สินค้า ${item.name} มีสต็อกไม่พอ (เหลือ ${product.stock})`);
        }
        ...
        if (paymentMethod === "เครดิต") {
          ...
          const dueDays = parseInt(document.getElementById("credit-due-days").value);
          if (!buyerName) throw new Error("กรุณาระบุชื่อผู้ซื้อสำหรับเครดิต");
          if (isNaN(dueDays)) throw new Error("กรุณาระบุจำนวนวันเครดิต");
          ...
          if (!transferorName) throw new Error("กรุณาระบุชื่อผู้โอน");
        }
        ...
        const newSale = {
          ...
          sellerId: this.currentUser.id,
          sellerName: this.currentUser.username,
          storeId: this.currentUser.storeId || null,
          storeName: document.getElementById("store-display-name").textContent.replace("- ", "") || null,
          buyerName: buyerName,
        ...
        this.cart = [];
        this.renderCart();
        this.renderPos(); // รีโหลดหน้า POS เพื่ออัปเดตสต็อกที่แสดง และเคลียร์ค่าต่างๆ
   ...
   renderSellerSalesHistoryWithFilter() {
      ...
      // ดึงค่าว่าเลือกแบบไหน (วันนี้ / วันที่ / ช่วง)
      // *หมายเหตุ: ต้องใช้ querySelector เพราะเราเปลี่ยน HTML ใหม่แล้ว
      ...
      // กำหนดช่วงเวลา Start - End ตามตัวเลือก
      if (filterType === "today") {
      ...
      if (typeof visibleDays === "number" && visibleDays >= 0) {
        ...
        if (filterStartDate < allowedDate) {
           tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:red; padding: 20px;">
                คุณได้รับอนุญาตให้ดูประวัติย้อนหลังได้ไม่เกิน ${visibleDays} วัน <br>
                (ดูได้ตั้งแต่วันที่ ${this.formatThaiDateShortYear(allowedDate)})
           </td></tr>`;
           return;
        }
      }
      ...
      mySales
        ...
        .forEach((sale) => {
          ...
          tr.innerHTML = `
                <td data-label="วันที่">${dateString}</td>
                <td data-label="เวลา">${timeString}</td>
                <td data-label="รายการสินค้า" style="text-align:left;">${itemsList}</td>
                <td data-label="ยอดขาย" style="font-weight:bold;">${this.formatNumberSmart(sale.total)}</td>
                <td data-label="ประเภทชำระ">${paymentDisplay}</td>
                <td data-label="จัดการ">
                    <button class="danger seller-delete-sale-btn" data-id="${sale.id}" style="padding: 5px 10px; font-size: 0.9em;">ลบ</button>
                </td>`;
          tbody.appendChild(tr);
        });
    },
    ...
    renderProductTable() {
      ...
      this.data.products.forEach((p) => {
        const tr = document.createElement("tr");
        // เพิ่มคอลัมน์บาร์โค้ด
        tr.innerHTML = `<td data-label="บาร์โค้ด">${p.barcode || "-"}</td> 
    ...
    saveProduct(e) {
      ...
      if (newProductData.barcode) {
          ...
          if (duplicate) {
              this.showToast(`บาร์โค้ดนี้ถูกใช้ไปแล้วกับสินค้า: ${duplicate.name}`, "error");
              return;
          }
      }
      ...
      if (id) {
        ...
        if (index > -1) {
          ...
          this.data.products[index].barcode = newProductData.barcode; // บันทึกบาร์โค้ด
        }
      } else {
    ...
    // --- [แก้ไข] ดึงข้อมูลบาร์โค้ดมาแสดงตอนแก้ไข ---
    editProduct(id) {
      const product = this.data.products.find((p) => p.id == id);
      if (product) {
        document.getElementById("product-id").value = product.id;
        // ใส่ค่าบาร์โค้ดลงในช่อง
        const barcodeInput = document.getElementById("product-barcode");
        if(barcodeInput) {
    ...
    fillPages() {
      document.getElementById("page-pos").innerHTML = `
        ...
        <div class="pos-layout">
            <div>
                <div id="scan-barcode-container" style="margin-bottom: 15px; background: #e3f2fd; padding: 10px; border-radius: 8px; border: 1px solid #90caf9;">
                    <label for="pos-barcode-input" style="display:block; text-align:left; font-weight:bold; color:#1565c0; margin-bottom:5px;">
                        <i class="fas fa-barcode"></i> สแกนบาร์โค้ดที่นี่:
                    </label>
                    <input type="text" id="pos-barcode-input" placeholder="คลิกแล้วยิงบาร์โค้ดเลย..." autocomplete="off" autofocus 
                           style="width:100%; font-size:1.2em; padding:10px; border: 2px solid #2196f3;">
                </div>

                <form id="add-to-cart-form" style="max-width:none;">
                    <div class="product-quantity-group" style="display:none;"> </div>
                    ...
                    <select id="pos-product"></select> <label for="pos-quantity">จำนวน:</label>
                    ...
                    <div class="form-actions">
                        <button type="submit" class="success">เพิ่มลงตะกร้า (Manual)</button>
      ...
      // 2. แก้ไขหน้า จัดการสินค้า เพิ่มช่องกรอก Barcode
      document.getElementById("page-products").innerHTML = `
        <h2>จัดการสินค้า</h2> 
        <p style="text-align:center; margin-top:-10px; margin-bottom:15px; font-size:0.9em;">กำหนดชื่อสินค้า หน่วยนับ และ <b>บาร์โค้ด</b></p>
        <form id="product-form"> 
            <input type="hidden" id="product-id"> 
            
            <label for="product-barcode">รหัสบาร์โค้ด:</label> 
            <input type="text" id="product-barcode" placeholder="ยิงบาร์โค้ดเพื่อบันทึก"> 
        ...
        <div class="table-container">
            <table id="product-table"> 
                <thead><tr><th>บาร์โค้ด</th><th>ชื่อสินค้า</th><th>สต็อก</th><th>หน่วย</th><th>จัดการ</th></tr></thead> 
                <tbody></tbody> 
            </table>
        </div>`;
        ...
        <form id="seller-sales-filter-form" style="max-width: none; background-color: #eef5ff; padding: 15px; border-radius: 8px; border: 1px solid #cce5ff;">
            ...
            <div style="display:flex; flex-wrap:wrap; gap: 20px; justify-content:center; align-items:center; margin-bottom: 15px;">
                <label style="cursor:pointer; font-weight:bold;">
                    <input type="radio" name="seller-filter-type" value="today" checked> วันนี้
                </label>
                <label style="cursor:pointer; font-weight:bold;">
                    <input type="radio" name="seller-filter-type" value="by_date"> เลือกวันที่
                </label>
                <label style="cursor:pointer; font-weight:bold;">
                    <input type="radio" name="seller-filter-type" value="by_range"> เลือกช่วงเวลา
                </label>
            </div>
            ...
            <div id="seller-date-inputs" style="display:flex; flex-direction:column; align-items:center; gap: 10px; min-height: 40px;">
                ...
                <div id="seller-filter-by-date-div" style="display:none;">
                    <label style="font-weight:normal;">วันที่ต้องการดู: 
...
attachEventListeners() {
      ...
      if (mainApp) {
        ...
        // ★★★ [เพิ่ม] Event Listener สำหรับช่อง Barcode ภายใน mainApp ★★★
        mainApp.addEventListener("keydown", (e) => {
            if (e.target.id === "pos-barcode-input" && e.key === "Enter") {
                e.preventDefault(); // ป้องกันการ Submit Form ปกติ
                const barcode = e.target.value.trim();
                if (barcode) {
                    this.handleBarcodeScan(barcode);
                }
                e.target.value = ""; // เคลียร์ค่าให้พร้อมยิงต่อทันที
            }
        });
      ...
      // 4. Keyboard Events (Global)
      document.addEventListener("keydown", (e) => {
        // [แก้ไข] ยกเว้นช่อง Barcode ออกจากการดักจับ Enter เพื่อยืนยันการขาย
        if (
          e.key === "Enter" &&
          e.target.tagName !== "INPUT" &&
          e.target.tagName !== "TEXTAREA" &&
          e.target.id !== "pos-barcode-input"
        ) {
